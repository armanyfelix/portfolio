  ...
  useMotionValueEvent(scrollYProgress, "change", (current) => {
    if (typeof current === "number") {
      ...
      if (scrollYProgress.get() < 0.05) {
        ...
        setBackground("bg-base-300/60 backdrop-blur");
        if (direction < 0) {
          setVisible(true);
        } else {
          setVisible(false);
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={cn(
          "flex max-w-fit fixed top-5 mx-auto inset-x-0 justify-self-center place-self-center border border-neutral/20 rounded-full shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-5000 pr-4 pl-8 py-2 items-center justify-center space-x-4",
          className,
          background,
        )}
      >
